...
type ReactFiberKey =
  "_reactFiber$... - random key here, this is just for typescript to properly catch the type";
...
describe("useStructuralId", () => {
  ...
  it("should return a structural id", () => {
    const { result } = renderHook(() => useStructuralId(null, []), {
  ...
  it("should find the right fiber node", () => {
    const { result } = renderHook(
      () =>
        useStructuralId(
          (node: Fiber) => node.elementType === components.Component3,
          [],
        ),
      { wrapper: TreeComponent },
    );
    ...
    const DivComponent = jest.fn(({ children }: Props) => {
      return (
        <span data-testid="FakeComponent3" ref={htmlNodeRef}>
          <OldComponent3>{children}</OldComponent3>
        </span>
      );
    });
